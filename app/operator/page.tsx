...
const INITIAL_FORM: FormData = {
  source: '', destination: '', distance: '', weight: '', priority: 'medium',
}
    ...
    try {
      const res = await fetch('/api/ai-plan', {
        ...
        body: JSON.stringify({
          ...
          distance: Number(form.distance), weight: Number(form.weight), priority: form.priority,
        }),
      })
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-7">
        <div className="flex items-start justify-between mb-7">
          ...
          <div className="flex items-center gap-6">
            <div className="text-center"><div className="text-3xl font-bold text-[#111111]">{shipments.length}</div><div className="text-xs text-zinc-400">Total Created</div></div>
            <div className="text-center"><div className="text-3xl font-bold text-[#111111]">{shipments.filter(s => s.status === 'pending').length}</div><div className="text-xs text-zinc-400">Pending</div></div>
            <div className="text-center"><div className="text-3xl font-bold text-brand-yellow">{shipments.filter(s => s.delay_risk === 'high').length}</div><div className="text-xs text-zinc-400">High Risk</div></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-5">
          ...
          <div className="col-span-4 card">
            ...
            <button onClick={handleAIPlan} disabled={planLoading}
              ...
              {planLoading ? <><Loader2 size={16} className="animate-spin" />AI Planning…</> : <><Sparkles size={16} />AI Plan Route</>}
            </button>
          </div>
